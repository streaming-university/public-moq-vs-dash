...
export function isSubscriber(m: Message): m is Subscriber {
    return m.kind == Msg.Subscribe || m.kind == Msg.Unsubscribe || m.kind == Msg.AnnounceOk || m.kind == Msg.AnnounceError
}
...
export function isPublisher(m: Message): m is Publisher {
    return m.kind == Msg.SubscribeOk || m.kind == Msg.SubscribeReset || m.kind == Msg.SubscribeError || m.kind == Msg.SubscribeFin || m.kind == Msg.Announce || m.kind == Msg.Unannounce
}
...
export interface Subscribe {
    ...
    switch_track_id?: bigint // track alias that will be switched
...
export class Decoder {
    ...
    private async msg(): Promise<Msg> {
        const t = (await this.r.u53()) as Id
        switch (t) {
            case Id.Subscribe:
        ...
        throw new Error(`unknown control message type: ${t as number}`)
    }
...
export class Encoder {
    ...
    async subscribe(s: Subscribe) {
        ...
        await this.w.u62(s.switch_track_id || BigInt(0))
