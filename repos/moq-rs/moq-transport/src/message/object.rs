...
pub struct Object {
	...
	/// ntp timestamp
	pub ntp_timestamp: Option<VarInt>,
}
...
impl Object {
	pub async fn decode<R: AsyncRead>(r: &mut R, extensions: &setup::Extensions) -> Result<Self, DecodeError> {
		...
		let ntp_timestamp = match extensions.ntp_timestamp {
		...
		// log::debug!("reading object: {:?}", ntp_timestamp);
	...
	pub async fn encode<W: AsyncWrite>(&self, w: &mut W, extensions: &setup::Extensions) -> Result<(), EncodeError> {
		...
		if extensions.ntp_timestamp {
			if let Some(ntp_timestamp) = self.ntp_timestamp {
				ntp_timestamp.encode(w).await?;
			}
		}
